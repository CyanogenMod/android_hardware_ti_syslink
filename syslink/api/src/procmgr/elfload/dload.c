...
int get_section_no(unsigned long target_addr);
...
/*                                                                           */
/*****************************************************************************/
static BOOL dload_and_allocate_dependencies(DLIMP_Dynamic_Module *dyn1_module)
{
   /*------------------------------------------------------------------------*/
...
/*                                                                           */
/*****************************************************************************/
static int load_object(LOADER_FILE_DESC *fd, DLIMP_Dynamic_Module *dyn1_module)
{
   /*------------------------------------------------------------------------*/
...
/*                                                                           */
/*****************************************************************************/
static void initialize_loaded_module(DLIMP_Dynamic_Module *dyn1_module)
{
   int i;
   ...
   loaded_module->name = malloc(1000);
#if 0
   strcpy(loaded_module->name, dyn_module->name);
   loaded_module->name = dyn_module->name ;
#endif
...
#if LOADER_DEBUG
         printf("%s:seg.phdr.p_vaddr 0x%x\n",__func__, seg.phdr.p_vaddr);
#endif
      }
...
static BOOL load_static_segment(LOADER_FILE_DESC *fd,
                                DLIMP_Dynamic_Module *dyn1_module)
{
   int i;
   ...
   for (j = 0; j < num_mem_entries; j++){
      ...
      if (status < 0) {
         printf("Memory_map failed for Physical Address 0x%x"
            "Exiting...\n",(UInt32)memory_regions[j].mpu_phys_addr);
         return FALSE;
      } else {
...
/*****************************************************************************/
static BOOL relocate_target_dynamic_tag_info(DLIMP_Dynamic_Module *dyn1_module,
                                             int i)
{
#ifdef ARM_TARGET
...
/*                                                                           */
/*****************************************************************************/
BOOL DLIMP_update_dyntag_section_address(DLIMP_Dynamic_Module *dyn1_module,
                                         int32_t i)
{
   int j;
   DLIMP_Loaded_Segment *seg = (DLIMP_Loaded_Segment *)
...
static DLIMP_Dynamic_Module *new_DLIMP_Dynamic_Module(LOADER_FILE_DESC *fd)
{
    ...
    /*-----------------------------------------------------------------------*/
    DLIMP_Dynamic_Module *dyn_module =
             (DLIMP_Dynamic_Module *)malloc(sizeof(DLIMP_Dynamic_Module));
...
/*                                                                           */
/*****************************************************************************/
static DLIMP_Loaded_Module *detach_loaded_module(DLIMP_Dynamic_Module *dyn1_module)
{
...
/*   it.                                                                     */
/*****************************************************************************/
static BOOL process_eiosabi(DLIMP_Dynamic_Module* dyn1_module)
{
#if ARM_TARGET
   if (is_arm_module(&dyn_module->fhdr))
                              ...
                              DLIMP_Dynamic_Module *dyn1_module)
{
   ...
   if (!file_header_machine_is_valid(dyn_module->fhdr.e_machine))
   {
	   ...
	   printf("dyn_module->fhdr.e_machine = 0x%x\n",dyn_module->fhdr.e_machine);
//       return 0;
   }
...
/*                                                                           */
/*****************************************************************************/
static BOOL find_dynamic_segment(DLIMP_Dynamic_Module *dyn1_module,
...
/*****************************************************************************/
static BOOL process_target_dynamic_tag(DLIMP_Dynamic_Module* dyn1_module, int i)
{
#ifdef ARM_TARGET
   if (is_arm_module(&dyn_module->fhdr))
                                  ...
                                  DLIMP_Dynamic_Module *dyn1_module)
{
   ...
   if (symtab_offset)
   {
      ...
      dyn_module->symtab =
                 malloc(dyn_module->symnum * sizeof(struct Elf32_Sym));
...
/*                                                                           */
/*****************************************************************************/
static void execute_module_pre_initialization(DLIMP_Dynamic_Module *dyn1_module)
{
   /*------------------------------------------------------------------------*/
...
/*                                                                           */
/*****************************************************************************/
static void execute_module_initialization(DLIMP_Dynamic_Module *dyn1_module)
{
   /*------------------------------------------------------------------------*/
...
int32_t DLOAD_load(LOADER_FILE_DESC *fd, int argc, char** argv)
{
   ...
   /*------------------------------------------------------------------------*/
	printf("Calling is_valid_elf_object_file %d\n",__LINE__);
   ...
   /*------------------------------------------------------------------------*/
	printf("Calling initialize_loaded_module %d\n",__LINE__);
   initialize_loaded_module(dyn_module);

   /*------------------------------------------------------------------------*/
   /* Append Module structure to loaded object list.                         */
   /*------------------------------------------------------------------------*/
	printf("Calling loaded_module_ptr_enqueue %d\n",__LINE__);
...
for(i=0; i< num_entries; i++){
	if (target_addr >= memory_regions[i].ducati_virt_addr && target_addr <(memory_regions[i].ducati_virt_addr + memory_regions[i].size))
		break;
	else
		continue;

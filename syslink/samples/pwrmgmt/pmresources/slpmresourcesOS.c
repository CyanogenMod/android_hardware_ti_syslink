 ...
 */
extern Int SlpmResources_startup (UInt32 notifyAddr, UInt32 sharedAddr);

/*!
 *  @brief  Function to execute the execute for SlpmResources sample application
 */
extern Int SlpmResources_execute (Void);

/*!
 *  @brief  Function to execute the shutdown for SlpmResources sample app
 */
extern Int SlpmResources_shutdown (Void);
...
main (int argc, char ** argv)
{
    ...
    if (traceClass != NULL) {
        ...
        status = SlpmResources_startup (0, 0);

    if (status >= 0) {
        SlpmResources_execute ();
    }

    SlpmResources_shutdown ();

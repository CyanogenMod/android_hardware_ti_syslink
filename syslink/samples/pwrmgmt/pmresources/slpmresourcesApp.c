 ...
 */
Int
SlpmResources_startup (UInt32 notifyAddr, UInt32 sharedAddr)
{
    Int32                          status  = 0;
#if defined(SYSLINK_USE_SYSMGR)
    ...
    Osal_printf ("Entered SlpmResources_startup sharedAddr [0x%x]\n",
                 sharedAddr);
    ...
    image_name = "./Resources_MPUSYS_Test_PM_Core0.xem3";
...
#ifdef SYSLINK_USE_LOADER
    image_name = "./Resources_MPUAPP_Test_PM_Core1.xem3";
    Osal_printf ("Loading image (%s) onto Ducati with ProcId %d\n", image_name,
            start_params.proc_id);
    status = ProcMgr_load (MessageQApp_procMgrHandle, image_name, 2,
                            (String *)image_name, &entry_point, &fileId,
    ...
    start_params.proc_id = MessageQApp_procId;
    image_name = "./MessageQ_MPUAPP_Test_Core1.xem3";
    Osal_printf ("Loading image (%s) onto Ducati with ProcId %d\n", image_name,
            start_params.proc_id);
    status = ProcMgr_load (MessageQApp_procMgrHandle, image_name, 2,
                            (String *)image_name, &entry_point, &fileId,
    ...
    Osal_printf ("Leaving SlpmResources_startup\n");
...
SlpmResources_execute (Void)
{
    ...
    if (status > 0) {
        ...
        for (i = 0 ; i < MESSAGEQAPP_NUM_TRANSFERS ; i++) {
            ...
            if (i==5) {
                status = ProcMgr_control (MessageQApp_procMgrHandle,
                                          PM_SUSPEND, NULL);
                status = ProcMgr_control (MessageQApp_procMgrHandle,
                                          PM_RESUME, NULL);
            }

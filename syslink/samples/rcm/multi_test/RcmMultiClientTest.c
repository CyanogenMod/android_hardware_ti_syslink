...
/*============================================================================
 *  @file   RcmMultiClientTest.c
 *
 *  @brief  OS-specific sample application framework for RCM module,
 *          with one server and multiple clients.
 *  ============================================================================
 */
...
RcmClient_Handle *      rcmClientHandles;
UInt                    numLocalClients;
UInt                    numRemoteClients;
pthread_t *             thread_client_exec; /*client exec thread objects */
pthread_t               thread_client;  /* client setup thread object */
UInt                    fxnDoubleIdx;
UInt                    fxnExitIdx;
...
HeapBuf_Handle                  heapHandle_app_heap;
...
Int32 fxnDouble(UInt32 dataSize, UInt32 *data)
{
    ...
    Osal_printf("Executing Remote Function fxnDouble\n");
...
Int32 fxnGetParams(UInt32 dataSize, UInt32 *data)
{
    ...
    args->params[0] = numRemoteClients;
...
Int GetSymbolIndex (Void)
{
    ...
    if(numLocalClients > 0) {
        ...
        for(i = 0; i < NUM_RCM_FUNCTIONS; i++) {
            Osal_printf("Querying server for %s() function index \n",
                fxnNames[i]);
            ...
            if (status < 0)
                ...
                Osal_printf("%s() symbol index [0x%x]\n", fxnNames[i],
                    fxnIndexes[i]);
        }
    }
...
Int TestExec (Void * arg)
{
    ...
    RcmClient_Handle rcmClientHandle = rcmClientHandles[clientNo];
    ...
    for (loop = 0; loop < NUM_RCM_FUNCTIONS ; loop++) {
        ...
        if(rcmMsg->result != loop * 2) {
            Osal_printf("TestExec(%d): called %s(%d), result = %d, expected"
                " = %d\n", clientNo, fxnNames[loop], fxnDoubleArgs->a,
                rcmMsg->result, loop * 2);
            status = -1;
            goto exit;
        }
...
Int TestExecNoWait(void)
{
    ...
    for (loop = 1; loop <= LOOP_COUNT; loop++) {
        ...
        for (job = 1; job <= JOB_COUNT; job++) {
            // allocate a remote command message
            rcmMsgSize = sizeof(RcmClient_Message) +
                sizeof(RCM_Remote_FxnArgs);
        ...
        for (job = 1; job <= JOB_COUNT; job++) {
            ...
            status = RcmClient_waitUntilDone (rcmClientHandle,
                msgIdAry[job-1], rcmMsg);
            if (status < 0) {
                Osal_printf("TestExecNoWait: RcmClient_waitUntilDone "
                    "error\n");
                goto exit;
            }
...
Int ipc_setup (Int testCase)
{
    ...
    if(status < 0) {
        Osal_printf("ipc_setup: Error in ProcMgr_load SysM3 image: "
            "[0x%x]\n", status);
        goto exit;
    }
...
Void RcmServerThreadFxn(Void *arg)
{
    ...
    if (status < 0) {
        Osal_printf("RcmServerThreadFxn: Error in RCM Server instance "
            "params init \n");
        goto exit;
    }
    ...
    Osal_printf("Registering remote function - fxnExit\n");
    status = RcmServer_addSymbol (rcmServerHandle, "fxnExit", fxnExit,
        &fxnIdx);
    if ((status < 0) || (fxnIdx == 0xFFFFFFFF)) {
        Osal_printf("Add symbol failed.\n");
        goto exit;
    ...
    for(i = 0; i < NUM_RCM_FUNCTIONS; i++) {
        Osal_printf("Registering remote function - %s\n", fxnNames[i]);
        status = RcmServer_addSymbol(rcmServerHandle, fxnNames[i],
            fxnDouble, &fxnIdx);
        if ((status < 0) || (fxnIdx == 0xFFFFFFFF)) {
            Osal_printf("Add symbol failed.\n");
            goto exit;
        }
        Osal_printf("Function %s registered with index %d\n", fxnNames[i],
            fxnIdx);
    }
    ...
    Osal_printf("Started RCM server thread. Reset the waitLoop to zero "
        "on the Ducati-side to continue the test...\n");
...
Void RcmClientThreadFxn(Void *arg)
{
    ...
    if (status < 0) {
        Osal_printf("RcmClientThreadFxn: Error in RCM Client module "
            "setup \n");
        goto exit;
    }
    ...
    if (status < 0) {
        Osal_printf("RcmClientThreadFxn: Error in RCM Client instance "
            "params init \n");
        goto exit;
    }
    ...
    Osal_printf("RcmClientThreadFxn: Opening RCM Server %s\n",
        remoteServerName);
    ...
    rcmClientHandles = (RcmClient_Handle *)Memory_alloc(NULL,
        numLocalClients * sizeof(RcmClient_Handle), sizeof(RcmClient_Handle));

    if(rcmClientHandles == NULL) {
        Osal_printf("RcmClientThreadFxn: Could not allocate handle "
            "array.\n");
        goto exit;
    }
    ...
    for(i = 0; i < numLocalClients; i++) {
        ...
        while ((rcmClientHandles[i] == NULL) &&
            ...
            Osal_printf("RcmClientThreadFxn: RCM Client create #%d passed\n",
                i);
        }

        if (MAX_CREATE_ATTEMPTS <= count) {
            Osal_printf("RcmClientThreadFxn: Timeout... #%d could not "
    ...
    Osal_printf("RcmClientThreadFxn: creating %d client execution threads.\n",
        numLocalClients);
    thread_client_exec = (pthread_t *) Memory_alloc(NULL,
        numLocalClients * sizeof(pthread_t), sizeof(pthread_t));
    if (thread_client_exec == NULL) {
        Osal_printf("RcmClientThreadFxn: Error in Memory_alloc \n");
        goto exit;
    }

    for(i = 0; i < numLocalClients; i++)
        pthread_create (&thread_client_exec[i], NULL, (Void *)&TestExec,
            (Void *) i);
    ...
    Memory_free(NULL, thread_client_exec, numLocalClients *
        sizeof(pthread_t));
...
Int RcmTestCleanup (Int testCase)
{
    ...
    if(numLocalClients > 0) {
        ...
        rcmClientHandle = rcmClientHandles[0];
        ...
        status = RcmClient_execDpc (rcmClientHandle, rcmMsg);
        if (status < 0) {
            Osal_printf("RcmTestCleanup: RcmClient_execDpc error. \n");
            goto exit;
        }
        ...
        Memory_free(NULL, rcmClientHandles, numLocalClients *
...
Int main (Int argc, Char * argv [])
{
    ...
    if (argc < 2)
    {
        Osal_printf("Usage: ./rcm_multiclienttest.out <Test#> <# local> "
            "<# remote>:\n");
        Osal_printf("\t./rcm_multiclienttest.out 1 : MPU <--> SysM3 "
            "testing\n");
        Osal_printf("\t./rcm_multiclienttest.out 2 : MPU <--> AppM3 "
            "testing\n");
        Osal_printf("<# local> = Number of clients on Chiron, defaults to "
            "1.\n");
        Osal_printf("<# remote> = Number of clients on Ducati, defaults to "
            "1.\n");
        goto exit;
    }
    ...
    if (argc > 2) {
        numLocalClients = atoi (argv[2]);
        Osal_printf("<# local> = %s => testing %d local clients\n",
            argv[2], numLocalClients);
    }
    else
        numLocalClients = 1;

    if (argc > 3) {
        numRemoteClients = atoi (argv[3]);
        Osal_printf("<# remote> = %s => testing %d remote clients\n",
            argv[3], numRemoteClients);
    }
    else
        numRemoteClients = 1;

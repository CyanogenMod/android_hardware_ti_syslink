...
Int ipc_setup (Int testCase)
{
...
#if defined(SYSLINK_USE_LOADER)
    if (testCase == 1)
        image_name = "./RCMSrvMultiClnt_MPUSYS_Test_Core0.xem3";
    else if (testCase == 2)
    ...
    if(status < 0) {
        Osal_printf("ipc_setup: Error in ProcMgr_load SysM3 image: [0x%x]\n",
            status);
        goto exit;
    }
    ...
    if(testCase == 2) {
#if defined(SYSLINK_USE_LOADER)
        image_name = "./RCMSrvMultiClnt_MPUAPP_Test_Core1.xem3";
        uProcId = MultiProc_getId (APPM3_PROC_NAME);
        status = ProcMgr_load (procMgrHandle, image_name, 2, &image_name,
                                &entry_point, &fileId, uProcId);
        if (status < 0) {
            Osal_printf("ipc_setup: Error in ProcMgr_load appM3 image: "
                "[0x%x]\n", status);
            goto exit;
        }
        ...
        status = ProcMgr_start (procMgrHandle, entry_point, &start_params);
        if (status < 0) {
            Osal_printf ("ipc_setup: Error in ProcMgr_start AppM3 [0x%x]\n",
                        status);
            goto exit;
        }
...
Int main (Int argc, Char * argv [])
{
    ...
    case 2:
        ...
        break;
    default:
        Osal_printf("main: Error testNo in RCM Client Server test \n");
        break;
    }

 ...
 *          SysM3
 */
#define RCM_MULTICLIENT_SYSM3ONLY_IMAGE  \
                                "./RCMSrvMultiClnt_MPUSYS_Test_Core0.xem3"

/*!
 *  @brief  Name of the SysM3 baseImage to be used for sample execution with
 *          AppM3
 */
#define RCM_MULTICLIENT_SYSM3_IMAGE      \
 ...
 *          AppM3
 */
#define RCM_MULTICLIENT_APPM3_IMAGE      \
                                "./RCMSrvMultiClnt_MPUAPP_Test_Core1.xem3"
...
Int GetSymbolIndex (Void)
{
    ...
    if (numLocalClients > 0) {
        ...
        Osal_printf ("\nGetSymbolIndex: Querying server for fxnDouble() "
                     "function index \n");
...
Int ipcSetup (Int testCase)
{
...
#if defined(SYSLINK_USE_LOADER)
    if (testCase == 1)
        imageName = RCM_MULTICLIENT_SYSM3ONLY_IMAGE;
    else if (testCase == 2)
        imageName = RCM_MULTICLIENT_SYSM3_IMAGE;
    ...
    if (testCase == 2) {
#if defined(SYSLINK_USE_LOADER)
        imageName = RCM_MULTICLIENT_APPM3_IMAGE;
...
Void RcmClientThreadFxn (Void *arg)
{
    ...
    Memory_free(NULL, threadClientExec, numLocalClients * sizeof(pthread_t));
...
Int RunTest (Int testCase)
{
    ...
    Osal_printf ("RunTest: Testing RCM Multiple Clients on MPU\n");
...
Void printUsage (Void)
{
    Osal_printf ("Usage: ./rcm_multiclienttest.out <Test#> [[<Local#>] "
        "[<Remote#>]]:\n");
    Osal_printf ("\t./rcm_multiclienttest.out 1 : MPU <--> SysM3 Sample\n");
    Osal_printf ("\t./rcm_multiclienttest.out 2 : MPU <--> AppM3 Sample\n");
    Osal_printf ("<Local#> = Number of clients on Chiron, defaults to 1.\n");
    Osal_printf ("<Remote#> = Number of clients on Ducati, defaults to 1.\n");
...
Int main (Int argc, Char * argv [])
{
    ...
    Osal_printf ("\nmain: == RCM Multiple Clients Sample ==\n");
    ...
    Osal_printf ("main: RCM multiple clients sample invoked\n");
    status = RunTest (testNo);
    if (status < 0)
        Osal_printf ("main: Error in RCM multiple clients sample\n");

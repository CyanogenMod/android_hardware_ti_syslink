...
Void RcmServerThreadFxn(Void *arg)
{
    ...
    if (status < 0) {
        Osal_printf("RcmServerThreadFxn: Error in RCM Server module "
            "setup\n");
        goto exit;
    }
...
Void RcmClientThreadFxn(Void *arg)
{
    ...
    Memory_free(NULL, thread_client_exec, numLocalClientThreads *
        sizeof(pthread_t));
...
Int main (Int argc, Char * argv [])
{
    ...
    if (argc < 2)
    {
        Osal_printf("Usage: ./rcm_multithreadtest.out <Test#> <# local> "
            "<# remote>:\n");
        Osal_printf("\t./rcm_multithreadtest.out 1 : MPU <--> SysM3 "
            "testing\n");
        Osal_printf("\t./rcm_multithreadtest.out 2 : MPU <--> AppM3 "
            "testing\n");
